...
// Thin wrapper over the Postmark mock REST API (docker-compose's postmarkmock service,
// port 3050) so tests can assert an email was actually sent rather than just trusting
// IEmailSender didn't throw.
public class PostmarkMockClient
{
    private readonly HttpClient _http;

    public PostmarkMockClient(string baseUrl = "http://localhost:3050")
    {
